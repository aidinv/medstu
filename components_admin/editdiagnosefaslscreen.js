...
export default class Editdiagnosefaslscreen extends Component {
    ...
    fetchData = async () => {
        try {
            const response = await fetch('https://draydinv.ir/extra/diagnosedetail.php', {
                method: 'POST',
                headers: {
    ...
    handleaddsubject = () => {
        ...
        fetch('https://draydinv.ir/extra/adddiagnosesubject.php', {
            method: 'POST',
            headers: {
    ...
    render() {
        ...
        const showDialog = (c, d) => {
            ...
            fetch('https://draydinv.ir/extra/deletediagnosesubject.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
        ...
        const ListItem = ({ text1, text2, text3, text4, onPress }) => (
            <TouchableOpacity style={styles.item} onPress={onPress} activeOpacity={0.9}>
                ...
                <View style={{ flexDirection: 'row-reverse' }}>
                    <TouchableOpacity onPress={() => navigation.navigate('Adddiagnosecontent', { name_fa: text1, name_en: text2, subject: text3, subject1: text4 })} >
                        <Image source={require('../assets/image/edit.png')} style={{ marginLeft: 5 }} resizeMode='contain' />
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => navigation.navigate('Changediagnosecontent', { name_fa: text1, name_en: text2, subject: text3, subject1: text4 })} >
        ...
        const SwipeableItem = ({ item }) => (
            <GestureHandlerRootView>
                <SwipeableFlatList
                    data={item}
                    renderItem={({ item }) =>
                        <ListItem text1={item.name_fa} text2={item.name_en} text3={item.subject} text4={item.subject1}
            ...
            <View style={{ flex: 1, justifyContent: 'flex-start', alignItems: 'center', marginTop: 60 }}>
                    ...
                    <View style={{ flexDirection: 'row-reverse', marginBottom: 10, justifyContent: 'space-around', width: this.width * 5 / 5, alignSelf: 'center' }}>
                        <View style={{ flexDirection: 'row-reverse', alignSelf: 'center' }}>
                            ...
                            <Text style={{ fontWeight: '900', color: '#D20062', textAlign: 'center', marginHorizontal: 3 }}> | {this.state.name_fa} </Text>
                        </View>
