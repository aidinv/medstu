...
export default class Diagnose_sign extends Component {
  ...
  async getMovies() {
    try {
      const response = await fetch('https://draydinv.ir/extra/diagnose_sign.php');
    ...
    return (
      <View style={styles.paginationContainer}>
        ...
        <Text style={{
          ...
          فهرست عناوین - صفحه  {currentPage}  از  {totalPages}
        </Text>
    ...
    const ListItem = ({ text, onPress, text1, likes, view }) => (
      <TouchableOpacity style={styles.item} onPress={onPress} activeOpacity={0.9}>
        ...
        <View style={{ flexDirection: 'row-reverse' }}>
          <Image source={require('../assets/image/bandage.png')} style={{ width: 30, height: 30, marginLeft: 5, justifyContent: 'center', alignSelf: 'center' }} />
          <View>
      ...
      <View style={{ flex: 1, marginTop: 10 }}>
        {isLoading ? (
          ...
          <SafeAreaView style={{ flex: 1 }}>
            ...
            <TextInput
              placeholder="جستجوی شکایت بالینی "
              value={searchQuery}
